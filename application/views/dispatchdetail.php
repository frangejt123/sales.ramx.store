...
$dropdownmenu = '';
if($dispatch["status"] == 0)//pending
	$dropdownmenu .= '<a class="dropdown-item dd-item text-warning" href="#" id="process_dispatch_btn"><i class="fa fa-truck"></i> &nbsp; For Delivery</a>';

if($dispatch["status"] == 1)
	$dropdownmenu .= '<a class="dropdown-item dd-item text-primary" href="#" id="complete_dispatch_btn"><i class="fa fa-check"></i> &nbsp; Delivered</a>';


$btnstatus = '<span class="pull-right span_seperator"></span>'.
				'<div class="dropdown pull-right">'.
					'<button id="status_btn" class="'.$btnarray["class"][$dispatch["status"]] .' lg-btn" data-toggle="dropdown">'.
						$btnarray["btn"][$dispatch["status"]].' &nbsp; '.$btnarray["text"][$dispatch["status"]].
					'</button>'.
...
<div id="container">
	<input type="text" hidden id="selected_dispatch" value="<?php echo $dispatch["id"]; ?>">
	...
	<div class="container-fluid">
		<form class="" id="dispatch_detail_form" method="post">
			<div class="form-group row">
				<input type="hidden" id="driver_id" value="<?php echo $dispatch["driver"]; ?>" />
				<label class="col-sm-1 col-form-label">Driver Name: </label>
				<div class="col-sm-4">
					<div class="form-group trx_driver filter_param">
						<div style="position: relative; height: 34px;">
							<input type="text" class="form-control driver_id" id="driver_id_autocomplete" style="position: absolute; z-index: 2; background: transparent;" value="<?php echo $dispatch["driver_name"]; ?>" <?php echo $inputattr; ?>>
				...
				<div class="col-sm-3">
					<input type="date" class="form-control" id="dispatch_date" value="<?php echo $dispatch["dispatch_date"]; ?>" <?php echo $inputattr; ?> />
				</div>
			</div>
		</form>
	</div>
						...
						<div class="box-body pt-2 border">
							...
							<table class="table table-striped table-hover" id="dispatchdetail_table">
								...
								foreach($dispatchdetail as $ind => $row){
									$transdate = date("mdY", strtotime($row["transaction_date"]));
									$formatID = $transdate.'-'.sprintf("%04s", $row["transaction_id"]);
									echo '<tr id="tr_'.$row["id"].'">';
									echo '<td hidden>'.$row["transaction_id"].'</td>';
									echo '<td>'.$formatID.'</td>';
